  ...
  useImperativeHandle(ref, () => ({
    getState: () => {
      return {
        ...
        membership_completion_date: dateOfCompletion
          ? formatDate(dateOfCompletion, "YYYY-MM-DD")
          : "",
      };
    },
  }));
  ...
  return (
    <CardComponent customStyle={styles.cardContainer}>
      ...
      <View style={isWebView ? styles.gridView : styles.gap}>
        ...
        {Boolean(isMembershipNumber) && !trainingDetails?.declaration_form && (
          <View style={styles.submitFormContainer}>
            <CommonText>
              {intl.formatMessage({ id: "label.submitDeclaration" })}
            </CommonText>
            ...
            >
              <CommonText
                customTextStyle={{ color: "#04AF55", fontWeight: 600 }}
              >
                {intl.formatMessage({ id: "label.submitDeclarationform" })}
              </CommonText>
              <CustomImage
      ...
      {showDeclarationModal && (
        <DeclarationForm
          intl={intl}
          onPressIconCross={() => setShowDeclarationModal(false)}
