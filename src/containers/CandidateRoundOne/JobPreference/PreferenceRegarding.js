...
const PreferenceRegarding = ({intl, isWebView, isViewMode = false, onValidationChange = () => {}}, ref) => {
  //states
  const [isPostedAnywhere, setIsPostedAnywhere] = useState(1);
  const [whetherTransferable, setWhetherTransferable] = useState(1);
  const [participatedInResidentialProgram, setParticipatedInResidentialProgram] = useState(1);
  ...
  //Lifecycle
  useEffect(() => {
    let res = expectedSalary.length > 0 && industryPreference.length > 0 && workAreaPreference.length > 0 && category.length > 0;
  ...
  }, [ isReadyToPlacedOutside, preferredRegion, expectedSalary, industryPreference, workAreaPreference, category, onValidationChange]);
    ...
    <CardComponent customStyle={styles.cardContainer}>
          ...
          <View style={isWebView ? styles.gridView : styles.gap}>
            <View style={styles.textInputContainer(isWebView)}>
              <CustomLabelView
                label={intl.formatMessage({ id: "label.posting_anywhere_in_india" })}
            ...
            <View style={styles.textInputContainer(isWebView)}>
              <CustomLabelView
                label={intl.formatMessage({ id: "label.whether_transferable_post_acceptable" })}
                isMandatory={!isViewMode}
              />
              <CustomToggleComponent
                isMandatory={!isViewMode}
            ...
            <View style={styles.textInputContainer(isWebView)}>
              <CustomLabelView
                label={intl.formatMessage({ id: "label.haveYouParticipatedInResidential" })}
                isMandatory={!isViewMode}
              />
              <CustomToggleComponent
                isMandatory={!isViewMode}
            ...
            <View style={styles.textInputContainer(isWebView)}>
              <CustomLabelView
                label={intl.formatMessage({ id: "label.ready_to_place_outside_india" })}
                isMandatory={!isViewMode}
              />
              <CustomToggleComponent
                isMandatory={!isViewMode}
            ...
            {!Boolean(isReadyToPlacedOutside) && <CustomTextInput
            ...
            <CustomTextInput
              ...
              placeholder={intl.formatMessage({ id: "label.expected_annual_salary" })}
          ...
          <View style={isWebView ? styles.twoColumnSingleElement : styles.gap}>
            <View style={{marginBottom: 24}}>
              <CustomTextInput
                  ...
                  isMandatory={!isViewMode}
                  customStyle={styles.textInputContainerWithoutBottomMargin(isWebView)}
                  isPaddingNotRequired
                  label={intl.formatMessage({ id: "label.preferences_kind_of_industry" })}
                  placeholder={intl.formatMessage({ id: "label.selectIndustryPreference" })}
                  isDropdown
                  options={BOOLEAN_OPTION}
                ...
                {industryPreference.map((item, index) => {
                  return(
                      <CustomChipCard
                        ...
                        onPress={() => {
                          let newPref = industryPreference.filter((i, ind) => index !== ind);
            ...
            <View style={{marginBottom: 24}}>
              <CustomTextInput
                  ...
                  customStyle={styles.textInputContainerWithoutBottomMargin(isWebView)}
                  isPaddingNotRequired
                  label={intl.formatMessage({ id: "label.preference_for_area_of_work" })}
                  placeholder={intl.formatMessage({ id: "label.selectWorkAreaPreference" })}
                  isDropdown
                  options={BOOLEAN_OPTION}
                ...
                {workAreaPreference.map((item, index) => {
                  return(
                      <CustomChipCard
                        ...
                        isEditable={!isViewMode}
                        onPress={() => {
                          let newPref = workAreaPreference.filter((i, ind) => index !== ind);
