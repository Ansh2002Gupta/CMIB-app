...
const WorkExperienceDetailsTemplate = (
  ...
  function convertExperienceArray(experienceArray) {
    return experienceArray.map((exp) => {
      return {
        has_work_experience: exp.isWorkExperience,
        organization_name: exp.orgName,
        designation: exp.designation,
        location: exp.location,
        from_date: formatDate(exp.from, "YYYY-MM-DD"),
        to_date: formatDate(exp.to, "YYYY-MM-DD"),
        emp_strength: Number(exp.empStrength),
        gross_salary_drawn: Number(exp.grossSalary),
        areas_of_work: exp.workAreas.split(",").map((area) => area.trim()),
      };
    });
  }
  ...
  const onDeleteWorkExperience = (LIndex) => {
    if (workExperiences.length > 1) {
      let newArticles = workExperiences.filter(
  ...
  useImperativeHandle(ref, () => ({
    getState: () => {
      return convertExperienceArray(workExperiences);
    },
  }));
                ...
                <View style={styles.textInputContainer(isWebView)}>
                  <CustomLabelView
                    label={intl.formatMessage({
                      id: "label.haveAnyWorkExperience",
                    })}
                  ...
                  <CustomToggleComponent
                    ...
                    onValueChange={(txt) =>
                      onDataChange(txt, index, "isWorkExperience")
                    }
                  />
                </View>
